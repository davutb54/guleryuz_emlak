...
function MobileTriggerContent({ initialFilters }: { initialFilters: FilterState }) {
  ...
  return (
    <>
      <button
        ...
        className="flex items-center gap-2 px-4 py-2.5 rounded-full border border-[rgba(212,167,68,0.25)] text-silver-300 text-sm font-medium hover:border-gold-500/50 hover:text-cream-100 transition-colors"
      >
      ...
      <Dialog.Root open={open} onOpenChange={setOpen}>
        <Dialog.Portal>
          ...
          >
            <Dialog.Title className="sr-only">Filtrele</Dialog.Title>
            <div className="flex justify-center pt-3 pb-1 shrink-0">
              <div className="w-10 h-1 rounded-full bg-silver-400/30" />
            </div>
            <div className="flex-1 overflow-y-auto px-5 pb-6 pt-2">
              <FilterPanel initialFilters={initialFilters} onClose={() => setOpen(false)} />
            </div>
          </Dialog.Content>
        </Dialog.Portal>
      </Dialog.Root>
...
/**
 * FilterSidebar — sadece desktop sidebar (lg+ görünür)
 */
export function FilterSidebar({ initialFilters }: { initialFilters: FilterState }) {
  return (
    <Suspense
      fallback={
...
/**
 * FilterMobileTrigger — sadece mobil buton + bottom sheet (lg'de gizli)
 */
export function FilterMobileTrigger({ initialFilters }: { initialFilters: FilterState }) {
  return (
    <Suspense fallback={null}>
      <MobileTriggerContent initialFilters={initialFilters} />
    </Suspense>
  );
}

/**
 * FilterSheet — her ikisini birden (eski davranış, geriye dönük uyumluluk)
 */
export default function FilterSheet({ initialFilters }: { initialFilters: FilterState }) {
  return (
    <>
